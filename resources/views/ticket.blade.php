...
@section('content')
    <div class="container p-5 ticketdiv dark-background">
            ...
            <div class="col-md-8 dark-background p-3">
                <div class="white-border p-3">
                    ...
                    @foreach ($ticketAttachments as $singleAttachment)
                        <a class="mx-2 specialfont" href="{{ Storage::url($singleAttachment->path) }}">Attachment
                            {{ $loop->iteration }} </a>
                    @endforeach
                </div>
            </div>
        ...
        <div class="row dark-background p-3">
            <h4 class="text-white text-center py-3" style="border-top:1px solid grey; border-bottom:1px solid grey;">Threads</h4>
        </div>
            ...
            <div class="col-md-12">
                ...
                <div style="border:1px solid grey; border-radius:10px;" class="mb-5">
                    <div class="comment p-3">
                        <div class="row d-flex align-items-center justify-content-center">
                            <div class="col-md-4">
                                <p class="specialfont">Added by - {{ App\Models\User::find($comment['user_id'])['name'] }}</p>
                            </div>
                            <div class="col-md-4">
                                <p class="specialfont">Added on - {{ $comment['created_at']->format('F j, Y h:i A') }}</p>
                            </div>
                            <div class="col-md-4">
                                <p class="specialfont">Email Id - {{ App\Models\User::find($comment['user_id'])['email'] }}</p>
                            </div>
                        </div>
                        <hr style="background:white;">
                        <div class="row p-2">
                            <div class="col-md-12 p-3" style="background:rgba(255,255,255,0.5); border-radius:10px; color:black;">
                                <p>{{ $comment['comment'] }}</p>
                            </div>
                        </div>
                    </div>
                ...
                @if (Session::has('uid'))
                    <form action="/add-comment" method="post">
                        ...
                        <button class="btn btn-danger my-3">Reply</button>
                        @error('comment')
                        <p class="text-danger">{{ $message }}</p>
                        @enderror
                    </form>
                    @else
                    <h5 class="text-white text-center">Please <a href="/login" style="font-weight:bold;" class="text-success">Login</a> to Comment !!!</h5>
                @endif
